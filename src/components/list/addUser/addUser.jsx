...
const AddUser = ({ onUserAdded /* Added */ }) => {
    ...
    <div className="p-5 bg-[#082742] border-[#18405f] shadow-lg rounded-xl border-2 absolute top-0 bottom-0 left-0 right-0 m-auto w-max h-max">
        <form onSubmit={handleSearch} className="flex gap-2 pb-3 items-center justify-center mb-2 border-b-2 border-[#18405f]">
       ...
       {user && <div className="flex justify-between mt-6">
            <div className="flex items-center gap-5">
                <img className="size-14 rounded-full" src={user.avatar || "src/assets/avatar.png"} alt=""/>
                <span className="font-semibold">{user.username}</span>
            </div>
            <button onClick={handleAdd} className="bg-black/25 px-6 rounded-md h-10 self-center">Add</button>
        </div>}
